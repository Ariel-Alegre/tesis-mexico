...
const createRedactionTesis = async (req, res) => {
  upload(req, res, async (err) => {
    ...
    try {
      ...
      const newRedactionTesis = await Correction.create({
        title,
        description,
        titleForm,
        titleForm2,
...
const updateRedactionTesis = async (req, res) => {
  upload(req, res, async (err) => {
    ...
    try {
      ...
      const RedactionTesisRecord = await Correction.findByPk(id);
...
const allRedactionTesis = async (req, res) => {
    ...
    try {
 const allredactiontesis = await Correction.findAll()
