const { ScientificArticle } = require('../db');
...
const createRedactionTesis = async (req, res) => {
  upload(req, res, async (err) => {
    ...
    try {
      ...
      const newRedactionTesis = await ScientificArticle.create({
        title,
        description,
        titleForm,
        titleForm2,
...
const updateRedactionTesis = async (req, res) => {
  upload(req, res, async (err) => {
    ...
    try {
      ...
      const RedactionTesisRecord = await ScientificArticle.findByPk(id);
...
const allRedactionTesis = async (req, res) => {
    ...
    try {
 const allredactiontesis = await ScientificArticle.findAll()
