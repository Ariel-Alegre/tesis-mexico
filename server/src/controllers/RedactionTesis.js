const { RedactionTesis } = require('../db');
...
const createRedactionTesis = async (req, res) => {
  upload(req, res, async (err) => {
    ...
    try {
      ...
      const newRedactionTesis = await RedactionTesis.create({
        title,
        description,
        titleForm,
        titleForm2,
...
const updateRedactionTesis = async (req, res) => {
  upload(req, res, async (err) => {
    ...
    try {
      ...
      const RedactionTesisRecord = await RedactionTesis.findByPk(id);
...
const allRedactionTesis = async (req, res) => {
    ...
    try {
 const allredactiontesis = await RedactionTesis.findAll()
